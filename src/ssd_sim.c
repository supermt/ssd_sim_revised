...
int main(int argc, char *argv[])
{
 ...
 strcpy(tracename, argv[1]);
 mcount = 0;
 printf("--->%s,%d", tracename);
 ...
 ssd = initiation(ssd, tracename, mcount);
 make_aged(ssd);
    ...
    for (k = 0; k < ssd->parameter->die_chip; k++)
    {
     for (l = 0; l < ssd->parameter->plane_die; l++)
     {
      ...
      for (m = 0; m < ssd->parameter->block_plane; m++)
      {
       ...
       ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].erase_count = ssd->parameter->ers_limit * ssd->parameter->aged_ratio * rand_value;
